                          ...
                          struct cpu_user_regs *regs)
{
 ...
 rewalk:
    ...
    src_page = get_page_from_gfn(d, (unsigned long)gfn, NULL, P2M_ALLOC);
    if ( unlikely(!src_page) )
    {
        okay = 0;
        printk("Error while copying from mfn %lx", gfn);
    }

    dst_page = get_page_from_gfn(d, (unsigned long)gfn, NULL, P2M_ALLOC);
    if ( unlikely(!dst_page) )
    {
        okay = 0;
        printk("Error while copying from mfn %lx", gfn);
    }
    ...
    //     if ( dst_page )
    //         put_page(dst_page);
    //     printk("Error while copying to mfn %lx", gfn);
    // }
